/**
  ******************************************************************************
  * @file TriangularDitherMethod/src/main.c 
...
#define TIM3_CC1_Address    ((uint32_t)0x40000434)
...
void TIM3_Configuration ( uint16_t PWM_period );
void DMA_TIM3_Configuration ( void );
#endif
...
int main(void)
{
 ...
 #ifndef Thermal_Noise_Measure 
  DMA_TIM3_Configuration();
  
   TIM3_Configuration( TIM_PWM_PERIOD );
...
/* Enable peripheral clocks --------------------------------------------------*/
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1 | RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOA, ENABLE);
    ...
    /* Enable Peripheral on ABP1 : Timet TIM3 Clock Enable*/
...
void GPIO_Configuration(void)
{
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure);
   ...
   /*GPIOA Configuration GPIOA.6: TIM3 channel 1 as output push pull ---------*/
...
/**
  ...
  *   sampling period                  
  * @param e required oversampling period in Ýseconds
  * @retval : None
  */
void TIM2_Configuration ( uint32_t Sampling_Period )
{  
...
void DMA_TIM3_Configuration ( void )
{
  ...
  DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)TIM3_CC1_Address;
  DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)TimerPulseWidth;
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;
  DMA_InitStructure.DMA_BufferSize = BUFFER_PW_WIDTH;
...
/**
  * @brief  Configures the TIM3 Channel 1 into PWM mode.
  * @param  None
  * @retval : None
  */
void TIM3_Configuration ( uint16_t PWM_period )
{
  ...
  /* -----------------------------------------------------------------------
  TIM3CLK = 56 MHz, Prescaler = 0x0, TIM3 counter clock = 56 MHz
  TIM3 ARR Register = 55 => TIM3 Frequency = TIM3 counter clock/(ARR + 1) 
  TIM3 Frequency = 1Mhz = ADC Conversion rate @14Mhz and 1.5Cycles as 
  sampling time
  ----------------------------------------------------------------------- */  
  /* Time base configuration */
