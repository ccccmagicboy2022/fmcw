...
void TIM3_Configuration ( uint32_t PWM_period );
...
void GPIO_Configuration(void)
{
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);
   ...
   /* Configure PA6: TIM3 channel 1 as output push pull ---------*/
...
void TIM3_Configuration ( uint32_t PWM_period )
{
  ...
  /* -----------------------------------------------------------------------
  TIM3CLK = 32 MHz, Prescaler = 0x0, TIM3 counter clock = 32 MHz
  TIM3 ARR Register = 55 => TIM3 Frequency = TIM3 counter clock/(ARR + 1)
  TIM3 Frequency = 1Mhz = ADC Conversion rate @16Mhz and 4 sampling cycles
  ----------------------------------------------------------------------- */
  /* Time base configuration */
