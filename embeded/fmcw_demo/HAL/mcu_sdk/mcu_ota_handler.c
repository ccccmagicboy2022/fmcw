...
static void mcu_ota_start_req(uint8_t*recv_data, uint32_t recv_len)
{
    ...
    length = set_bt_uart_buffer(length, (uint8_t *)p_buf, payload_len);
...
static void mcu_ota_file_info_req(uint8_t*recv_data, uint32_t recv_len)
{
    ...
    if (memcmp(&recv_data[0], PRODUCT_KEY, 8) == 0) {
        if ((file_version > MCU_OTA_VERSION) && (file_length <= APP_NEW_FW_MAX_SIZE)) {
            if (file_md5&&(s_dfu_settings.progress.firmware_file_version == file_version) &&
               ...
               (s_dfu_settings.progress.firmware_file_crc == file_crc)) {
                ...
                state = 0;
                mcu_flash_write(DFU_SETTING_SAVE_ADDR,
                        (uint8_t*)&s_dfu_settings,
                        sizeof(s_dfu_settings));
            }
...
uint32_t mcu_ota_init(void)
{
    ...
    current_package = 0;
    last_package = 0;
    mcu_flash_read(DFU_SETTING_SAVE_ADDR,
                   (uint8_t*)&s_dfu_settings,
                   sizeof(s_dfu_settings));
...
void write_ota_flag(uint16_t cmd)
{
    uint32_t u32Addr;
...
//    if (FLASH_COMPL != FLASH_EraseOnePage(FLASH_SECTOR61_ADDR))
//    {
//        //error
//    }

//    if (FLASH_COMPL != FLASH_ProgramWord(FLASH_SECTOR61_ADDR, 0x0000FFFFu & cmd))
//    {
//        //error
//    }
...
void jump_to_bootloader(uint16_t cmd)
{
    write_ota_flag(cmd);
    soft_reset_mcu();
}

static void mcu_ota_start_req(uint8_t*recv_data,
                              uint32_t recv_len,
                              uint16_t cmd)
{
    jump_to_bootloader(cmd);
}
...
void mcu_ota_proc(uint16_t cmd, uint8_t*recv_data, uint32_t recv_len)
{
    ...
    case PHOSENSE_BCI_UART_COMMON_MCU_OTA_REQUEST:
        mcu_ota_start_req(recv_data, recv_len,  cmd);
        break;
    default:
        TUYA_OTA_LOG("tuya_ota_proc cmd err.");
        break;
