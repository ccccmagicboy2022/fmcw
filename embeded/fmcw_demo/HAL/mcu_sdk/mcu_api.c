...
uint8_t mcu_dp_raw_update(uint8_t dpid, const uint8_t value[], uint16_t len)
{
    ...
    length = set_bt_uart_buffer(length, (uint8_t *)value, len);
...
{
    bt_uart_tx_buf[DATA_START] = channel;
    bt_uart_write_frame(ZIGBEE_RF_TEST_CMD, 1);
}
...
void mcu_reset_zigbee(void)
{
    ...
    length = set_bt_uart_byte(length, 0);
    bt_uart_write_frame(ZIGBEE_CFG_CMD, length);
}

/**
* @brief mcu start zigbee module
* this commond must be call after reveived checking proudect info, or after get zigbee network info
* @param[in]  {void}
* @return void
*/
...
void uart_receive_input(uint8_t value)
{
    if ((queue_in > queue_out) && ((queue_in - queue_out) >= sizeof(bt_queue_buf))) {
        ...
        // Queue is not full
        if (queue_in >= (uint8_t *)(bt_queue_buf + sizeof(bt_queue_buf))) {
            queue_in = (uint8_t *)(bt_queue_buf);
        }
...
static uint8_t mcu_common_uart_data_unpack(uint8_t data)
{
    ...
    if ((bt_uart_rx_buf_temp[0] == 0x55) &&
        (bt_uart_rx_buf_temp[1] == 0xAA) &&
        (bt_uart_rx_buf_temp[2] == 0x00)) {
    ...
    case MCU_UART_REV_STATE_FOUND_DATA:
        for (i = 0; i < UART_RX_Count; i++)
            checksum += bt_uart_rx_buf[i];
        if (data == checksum) {
            bt_uart_rx_buf[UART_RX_Count++] = data;  // sum
            ret = TRUE;
        } else {
            my_memset(bt_uart_rx_buf_temp, 0, 3);
            my_memset(bt_uart_rx_buf, 0, sizeof(bt_uart_rx_buf));
...
//  uint16_t offset = 0;
//  volatile  uint16_t rx_value_len = 0;             // Data frame length

    if ((UART_RX_Count < sizeof(bt_uart_rx_buf)) && get_queue_total_data() > 0) {
...
void zigbee_uart_service(void)
{
    ...
    static uint16_t rx_in = 0;
    uint16_t offset = 0;
    uint16_t rx_value_len = 0;
    uint16_t protocol_version = 0;

    while ((rx_in < sizeof(bt_uart_rx_buf)) && (with_data_rxbuff() > 0)) {
    ...
    while ((rx_in - offset) >= PROTOCOL_HEAD) {
        if (bt_uart_rx_buf[offset + HEAD_FIRST] != 0x55) {
            offset++;
            continue;
        }

        if (bt_uart_rx_buf[offset + HEAD_SECOND] != 0xaa) {
            offset++;
            continue;
        }

        protocol_version = bt_uart_rx_buf[offset + PROTOCOL_VERSION];
        /*version 2 is more than two byte  sequence number than version 1*/
        if (protocol_version != VERSION) {
            offset += 2;
            continue;
        }

        rx_value_len =  bt_uart_rx_buf[offset + LENGTH_HIGH ] * 0x100;
        rx_value_len += (bt_uart_rx_buf[offset + LENGTH_LOW] + PROTOCOL_HEAD);

        if (rx_value_len > sizeof(bt_uart_rx_buf)) {
            offset += 3;
            continue;
        }
        ...
        if (get_check_sum((uint8_t *)bt_uart_rx_buf + offset, rx_value_len - 1) !=
            bt_uart_rx_buf[offset + rx_value_len - 1]) {
            offset += 3;
            continue;
        }
    ...
    if (rx_in > 0) {
    if ((offset == 0) && (rx_in >= sizeof(bt_uart_rx_buf))) {
            offset++;
            rx_in -= offset;
        }
        my_memcpy((char*)bt_uart_rx_buf, (const char *)bt_uart_rx_buf + offset, rx_in);
    }
}
